...
namespace MyCampusV2.Controllers
{
    ...
    public class DepartmentController : BaseController
    {
        ...
        public DepartmentController(IDepartmentService departmentService, IMapper mapper, IAuditTrailService audit, IBatchUploadService batchUploadService, IOptions<BatchUploadSettings> batchSettings, IOptions<ExcelHelper> excelHelper)
        {
        ...
        //[CustomAuthorize]
        [HttpGet]
        [Route("getDepartmentsUsingCampusId/{id}")]
        public async Task<IList<departmentVM>> GetDepartmentsUsingCampusId(int id)
        {
            return _mapper.Map<IList<departmentVM>>(await _departmentService.GetDepartmentsUsingCampusId(id));
        }
        ...
        {
            return _mapper.Map<departmentVM>(await _departmentService.GetDepartmentById(id));
        }
        ...
        [HttpGet]
        [Route("all")]
        public async Task<departmentPagedResultVM> GetAll([FromQuery] PaginationParams param)
        {
            return _mapper.Map<departmentPagedResultVM>(await _departmentService.GetAllDepartment(param.PageNo, param.PageSize, param.Keyword));
        }
        ...
        [HttpPost]
        [Route("create")]
        public async Task<ResultModel> Create([FromBody] departmentVM department)
        {
            try
            {
                department.addedBy = GetUserId();
                department.updatedBy = GetUserId();
                return await _departmentService.AddDepartment(_mapper.Map<departmentEntity>(department));
            }
            catch (Exception err)
            {
        ...
        [HttpPut]
        [Route("update")]
        public async Task<ResultModel> Update([FromBody] departmentVM department)
        {
            try
            {
                department.updatedBy = GetUserId();
                return await _departmentService.UpdateDepartment(_mapper.Map<departmentEntity>(department));
            }
            catch (Exception err)
            {
                return CreateResult("500", err.Message, false);
        ...
        [CustomAuthorize]
        [HttpDelete]
        [Route("permanentDeleteDepartment/{id}")]
        public async Task<ResultModel> PermanentDelete(int id)
        {
            try
            {
                return await _departmentService.DeleteDepartmentPermanent(id, GetUserId());
            }
            catch (Exception err)
            {
                return CreateResult("500", err.Message, false);
        ...
        [CustomAuthorize]
        [HttpDelete]
        [Route("temporaryDeleteDepartment/{id}")]
        public async Task<ResultModel> TemporaryDelete(int id)
        {
            try
            {
                return await _departmentService.DeleteDepartmentTemporary(id, GetUserId());
            }
            catch (Exception err)
            {
                return CreateResult("500", err.Message, false);
        ...
        public async Task<ResultModel> RetrieveData(int id)
        {
            try
            {
                ...
                return await _departmentService.RetrieveDepartment(_mapper.Map<departmentEntity>(entity));
            }
            catch (Exception err)
            {
                return CreateResult("500", err.Message, false);
        ...
        public async Task<IActionResult> DownloadTemplateDepartment([FromQuery] PaginationParams param, string type)
        {
            try
            {
                byte[] file = null;
                file = await Task.Run(() => _excelHelper.ExportTemplate(ExcelVar.DepartmentColHeader, ExcelVar.DepartmentSampleData, ExcelVar.DepartmentTemplateTitle));
        ...
        [HttpGet]
        [Route("exportdepartmentexcelfile")]
        public async Task<IActionResult> ExportDepartmentExcelFile([FromQuery] PaginationParams param)
        {
            try
            {
                var result = _mapper.Map<departmentPagedResultVM>(await _departmentService.ExportAllDepartments(param.Keyword == null ? string.Empty : param.Keyword));
                byte[] file = null;
                ...
                if (result.departments.Count != 0)
                {
                    ...
                    {
                        string[] ColHeader = ExcelVar.DepartmentColHeader;
                        string wsTitle = ExcelVar.DepartmentTitle;
                        ...
                        int rowNumber = 2;
                        foreach (departmentVM row in result.departments)
                        {
        ...
        public async Task<IActionResult> BatchUploadProcess(int id)
        {
            ...
            try
            {
                ...
                IEnumerable<departmentBatchUploadVM> records = _batchUploadService.GetDepartmentRecords(entity.Path);
                var process = records.Skip(entity.ProcessCount).Take(_batchSettings.ProcessCount).ToList();
                // Do the update process here.
                response = await _departmentService.BatchUpload(process, GetUserId(), entity.ID, lastProcess);
        ...
        public async Task<IActionResult> BatchUpload(IFormFile file)
        {
            ...
            try
            {
                entity.Form_ID = (int)Form.Campus_Department;
